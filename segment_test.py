...
class MyTest(unittest.TestCase):
    ...
    def test_006(self):
        """ headings """
        with open("tests/segment_heading.txt", "r") as f:
            text = f.read()
        segment = Segment(text)
        self.assertEquals(segment.segments, [{'tag': 1, 'text': 'TABLE OF CONTENTS'}, {'tag': 1, 'text': '1. Section One'}, {'tag': 1, 'text': '2. Section Two'}])
        
    def test_007(self):
        """ page nimbering - top of page """
        segment = Segment("Page 1\nfirst\nsecond\n\n")
        self.assertEquals(segment.segments, [{'tag': 3, 'text': 'Page 1'}, {'tag': 2, 'text': 'first\nsecond'}])     

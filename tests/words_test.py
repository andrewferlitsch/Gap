...
class MyTest(unittest.TestCase):
    ...
    def test_291(self):
        ...
        words = Words("handful powerful shitful artful beautiful colorful fanciful merciful", sentiment=True)
    ...
    def test_318(self):
        """ more sentiment """
        words = Words("like likes fun", sentiment=True)
        self.assertEqual(words.words, [{'word': 'like', 'tag': 18}, {'word': 'like', 'tag': 18}, {'word': 'fun', 'tag': 18}])
        words = Words("dead", sentiment=True)
        self.assertEqual(words.words, [{'word': 'dead', 'tag': 19}])
        words = Words("idiot", sentiment=True)
        self.assertEqual(words.words, [{'word': 'idiot', 'tag': 19}])
        words = Words("idiots", sentiment=True)
        self.assertEqual(words.words, [{'word': 'idiot', 'tag': 19}])
        words = Words("idiotic", sentiment=True)
        self.assertEqual(words.words, [{'word': 'idiot', 'tag': 19}])
        words = Words("dumb", sentiment=True)
        self.assertEqual(words.words, [{'word': 'dumb', 'tag': 19}])
        words = Words("dumber", sentiment=True)
        self.assertEqual(words.words, [{'word': 'dumb', 'tag': 19}])
        words = Words("dumbest", sentiment=True)
        self.assertEqual(words.words, [{'word': 'dumb', 'tag': 19}])
        words = Words("ugly", sentiment=True)
        self.assertEqual(words.words, [{'word': 'ugly', 'tag': 19}])
        words = Words("ugliest", sentiment=True)
        self.assertEqual(words.words, [{'word': 'ugly', 'tag': 19}])
        words = Words("stupid", sentiment=True)
        self.assertEqual(words.words, [{'word': 'stupid', 'tag': 19}])
        words = Words("stupidest", sentiment=True)
        self.assertEqual(words.words, [{'word': 'stupid', 'tag': 19}])
        words = Words("shit", sentiment=True)
        self.assertEqual(words.words, [{'word': 'shit', 'tag': 19}])
        
    def test_319(self):
        """ more sentiment """
        words = Words("dumbiest", sentiment=True)
        self.assertEqual(words.words, [{'word': 'dumb', 'tag': 19}])
        words = Words("stupidity", sentiment=True)
        self.assertEqual(words.words, [{'word': 'stupid', 'tag': 19}])
        words = Words("stupidified", sentiment=True)
        self.assertEqual(words.words, [{'word': 'stupid', 'tag': 19}])
        words = Words("shithole", sentiment=True)
        self.assertEqual(words.words, [{'word': 'shit', 'tag': 19}])
        words = Words("stink", sentiment=True)
        self.assertEqual(words.words, [{'word': 'stink', 'tag': 19}])
        words = Words("awesome liked", sentiment=True)
        self.assertEqual(words.words, [{'word': 'awesome', 'tag': 18}, {'word': 'like', 'tag': 18}])
        words = Words("amaze amazed amazing", sentiment=True)
        self.assertEqual(words.words, [{'word': 'amaze', 'tag': 18}, {'word': 'amaze', 'tag': 18}, {'word': 'amaze', 'tag': 18}])
        words = Words("pleasant beauty beautiful", sentiment=True)
        self.assertEqual(words.words, [{'word': 'pleasant', 'tag': 18}, {'word': 'beauty', 'tag': 18}, {'word': 'beauty', 'tag': 18}])
        words = Words("crap", sentiment=True)
        self.assertEqual(words.words, [{'word': 'crap', 'tag': 19}])
        words = Words("crappy", sentiment=True)
        self.assertEqual(words.words, [{'word': 'crap', 'tag': 19}])
        words = Words("crappiest", sentiment=True)
        self.assertEqual(words.words, [{'word': 'crap', 'tag': 19}])
        words = Words("sucks", sentiment=True)
        self.assertEqual(words.words, [{'word': 'suck', 'tag': 19}])
        words = Words("cool", sentiment=True)
        self.assertEqual(words.words, [{'word': 'cool', 'tag': 18}])
    ...
    def test_327(self):
        ...
        words = Words("behaviour", stopwords=True)
        self.assertEqual(words.words, [{'word': 'behavior', 'tag': 0}])
        words = Words("theatre", stopwords=True)
        self.assertEqual(words.words, [{'word': 'theater', 'tag': 0}])
        words = Words("grey", stopwords=True)
        self.assertEqual(words.words, [{'word': 'gray', 'tag': 0}])
        words = Words("programme", stopwords=True)
        self.assertEqual(words.words, [{'word': 'program', 'tag': 0}])
        words = Words("jewellery", stopwords=True)
        self.assertEqual(words.words, [{'word': 'jewelery', 'tag': 0}])
        words = Words("collectable", stopwords=True)
        self.assertEqual(words.words, [{'word': 'collectible', 'tag': 0}])
        words = Words("defence", stopwords=True)
        self.assertEqual(words.words, [{'word': 'defense', 'tag': 0}])
        words = Words("licence", stopwords=True)
        self.assertEqual(words.words, [{'word': 'license', 'tag': 0}])
        words = Words("dialogue", stopwords=True)
        self.assertEqual(words.words, [{'word': 'dialog', 'tag': 0}])
        words = Words("cheque", stopwords=True)
        self.assertEqual(words.words, [{'word': 'check', 'tag': 0}])
        words = Words("flavour", stopwords=True)
        self.assertEqual(words.words, [{'word': 'flavor', 'tag': 0}])
        words = Words("humour", stopwords=True)
        self.assertEqual(words.words, [{'word': 'humor', 'tag': 0}])
        words = Words("labour", stopwords=True)
        ...
        words = Words("breathalyse", stopwords=True)
        self.assertEqual(words.words, [{'word': 'breathalyse', 'tag': 0}])
        words = Words("paralyse", stopwords=True)
        self.assertEqual(words.words, [{'word': 'paralyze', 'tag': 0}])
        words = Words("traveller travelled travelling", stopwords=True)
        self.assertEqual(words.words, [{'word': 'travel', 'tag': 0}, {'word': 'travel', 'tag': 0}, {'word': 'travel', 'tag': 0}])
        words = Words("fuelled fuelling", stopwords=True)
        self.assertEqual(words.words, [{'word': 'fuel', 'tag': 0}, {'word': 'fuel', 'tag': 0}])
        words = Words("leukaemia", stopwords=True)
        self.assertEqual(words.words, [{'word': 'leukemia', 'tag': 0}])
        words = Words("oestrogen", stopwords=True)
        self.assertEqual(words.words, [{'word': 'estrogen', 'tag': 0}])
        words = Words("peaditric", stopwords=True)
        self.assertEqual(words.words, [{'word': 'peditric', 'tag': 0}])
        words = Words("manoeuvre", stopwords=True)
        self.assertEqual(words.words, [{'word': 'maneuver', 'tag': 0}])
        words = Words("pretence", stopwords=True)
        self.assertEqual(words.words, [{'word': 'pretense', 'tag': 0}])
        words = Words("catalogue", stopwords=True)
        self.assertEqual(words.words, [{'word': 'catalog', 'tag': 0}])
        words = Words("analogue", stopwords=True)
        self.assertEqual(words.words, [{'word': 'analog', 'tag': 0}])
        words = Words("parlour honour favour honourable", stopwords=True)
        self.assertEqual(words.words, [{'word': 'parlor', 'tag': 0}, {'word': 'honor', 'tag': 0}, {'word': 'favor', 'tag': 0}, {'word': 'honor', 'tag': 0}])
        words = Words("favourable", stopwords=True)
        self.assertEqual(words.words, [{'word': 'favor', 'tag': 0}])
        words = Words("endeavour", stopwords=True)
        self.assertEqual(words.words, [{'word': 'endeavor', 'tag': 0}])
        words = Words("harbour", stopwords=True)
        self.assertEqual(words.words, [{'word': 'harbor', 'tag': 0}])
        words = Words("splendour", stopwords=True)
        self.assertEqual(words.words, [{'word': 'splendor', 'tag': 0}])
        words = Words("marvellous", stopwords=True)
        self.assertEqual(words.words, [{'word': 'marvelous', 'tag': 0}])
        words = Words("vigour", stopwords=True)
        self.assertEqual(words.words, [{'word': 'vigor', 'tag': 0}])
        words = Words("practise practised practising", stopwords=True)
        self.assertEqual(words.words, [{'word': 'practice', 'tag': 0}, {'word': 'practice', 'tag': 0}, {'word': 'practice', 'tag': 0}])
        words = Words("despatch", stopwords=True)
        self.assertEqual(words.words, [{'word': 'dispatch', 'tag': 0}])
        words = Words("plough", stopwords=True)
        self.assertEqual(words.words, [{'word': 'plow', 'tag': 0}])
        words = Words("leapt", stopwords=True)
        self.assertEqual(words.words, [{'word': 'leap', 'tag': 0}])
        words = Words("leant", stopwords=True)
        self.assertEqual(words.words, [{'word': 'lean', 'tag': 0}])
        words = Words("dreamt", stopwords=True)
        self.assertEqual(words.words, [{'word': 'dream', 'tag': 0}])
        words = Words("learnt", stopwords=True)
        self.assertEqual(words.words, [{'word': 'learn', 'tag': 0}])
        words = Words("odour", stopwords=True)
        self.assertEqual(words.words, [{'word': 'odor', 'tag': 0}])
        words = Words("rumour", stopwords=True)
        self.assertEqual(words.words, [{'word': 'rumor', 'tag': 0}])
        words = Words("mediaeval", stopwords=True)
        self.assertEqual(words.words, [{'word': 'medieval', 'tag': 0}])
        words = Words("meagre", stopwords=True)
        self.assertEqual(words.words, [{'word': 'meager', 'tag': 0}])
        words = Words("sceptre", stopwords=True)
        self.assertEqual(words.words, [{'word': 'scepter', 'tag': 0}])
        words = Words("succour", stopwords=True)
        self.assertEqual(words.words, [{'word': 'succor', 'tag': 0}])
        words = Words("vapour", stopwords=True)
        self.assertEqual(words.words, [{'word': 'vapor', 'tag': 0}])
    ...
    def test_328(self):
        ...
        self.assertEqual(words.words, [{'word': 'organize', 'tag': 0}, {'word': 'organize', 'tag': 0}, {'word': 'organize', 'tag': 0}])
        words = Words("analyses analysis", stopwords=True)
        self.assertEqual(words.words, [{'word': 'analyze', 'tag': 0}, {'word': 'analyze', 'tag': 0}])
        words = Words("unfavorable", stopwords=True)
        self.assertEqual(words.words, [{'word': 'unfavorable', 'tag': 0}])

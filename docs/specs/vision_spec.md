...
If the augment property is set to True, for each image in the training set, an additional image is generated by rotating the image a random value between -90 and 90 degrees. Thus, if the training set is 1000 images, the `next()` operator will iterate through 2000 images.
